...
```{r pm_f Y corrplot, fig.align='center', fig.cap="**Figure M7.** Correlation matrix displaying *Spearman*'s $\\rho$ for all computed F metrics and our response variables on our training set. Only significant correlations are displayed (with $\\alpha$ = 0.05)", warning=FALSE}
...
```{r pm clutchsize scatterF, fig.align='center', fig.cap="**Figure M8a.** Bivariate scatterplot between *Parus major*'s clutch size and each potential connectivity metric, with and without log-tranformation", fig.width=12, fig.height=9, message=FALSE, comment=NA, warning=FALSE}
...
```{r pm mass scatterF, fig.align='center', fig.cap="**Figure M8b.** Bivariate scatterplot between the mass of *Parus major* nestlings and each potential connectivity metric, with and without log-tranformation", fig.width=12, fig.height=9, message=FALSE, comment=NA, warning=FALSE}
