...
We then investigated the bivariate correlations among the F metrics and some other predictors:
...
\
To further study distributions and bivariate correlations, we decided to plot every numeric predictor against each others. It gave us the possibility to start investigating potential **multivariate outliers**.
...
So, up to this point, we'd be rather inclined into **discarding** *F metrics* based on the *longest distances*, and we have a slight preference for *F metrics* computed with *beta* = 0.
\
\
...
trainset_pm %>% dplyr::select(id_nestbox, site, breeding_window, clutch_size, brood_size, fledgling_nb,
                       mass, tarsus_length, wing_length, father_cond, mother_cond, woodyveg_volume,
                       pmF_d113_beta0, pmF_d531_beta0, pmF_d1368_beta0, pmF_d2848_beta0, 
                       pmF_d113_beta1, pmF_d531_beta1, pmF_d1368_beta1, pmF_d2848_beta1) -> trainset_pm
...
1. Most *response variables* appear **unrelated** to our *F metrics* (according to Spearman's rank correlations), with the exception of *clutch size* that is **mildly negatively correlated** with several  *short distance F metrics*.
2. Our *morphometric response variables* are **mildly positively correlated** with our *beta0 F metrics*, except *wing length* that is also **correlated** with the *beta1 F* ones. 
3. Surprisingly, the *parental condition variables* (that are covariates) display a very peculiar pattern with *father_cond* being **positively correlated** with our *connectivity metrics* while the reverse is partially true for *mother_cond*.

To further examine these relationships, we plotted the Great tits couples' *clutch size* and the *mass* of nestlings against all our computed *F metrics*:
...
tpm_xs <- tpm_x[,c("clutch_size", "pmF_d113_beta0", "pmF_d531_beta0", "pmF_d1368_beta0", "pmF_d2848_beta0", "pmF_d113_beta1", "pmF_d531_beta1", "pmF_d1368_beta1", "pmF_d2848_beta1")]

# Transforming predictors:
tpm_xs <- tpm_xs %>%
  dplyr::mutate("pmF_d113_beta0 (log)" = log(pmF_d113_beta0)) %>%
  dplyr::mutate("pmF_d531_beta0 (log)" = log(pmF_d531_beta0)) %>%
  dplyr::mutate("pmF_d1368_beta0 (log)" = log(pmF_d1368_beta0)) %>%
  dplyr::mutate("pmF_d2848_beta0 (log)" = log(pmF_d2848_beta0)) %>%
  dplyr::mutate("pmF_d113_beta1 (log)" = log(pmF_d113_beta1)) %>%
  dplyr::mutate("pmF_d531_beta1 (log)" = log(pmF_d531_beta1)) %>%
  dplyr::mutate("pmF_d1368_beta1 (log)" = log(pmF_d1368_beta1)) %>%
  dplyr::mutate("pmF_d2848_beta1 (log)" = log(pmF_d2848_beta1)) # These transformations were made to linearise the relationships.

predictors <- colnames(tpm_xs)[2:ncol(tpm_xs)]
# Bind the clutch_size and tidying the data for plot (ggplot2, so long format):
mydata <- tpm_xs %>%
  tidyr::gather(key = "F_metric", value = "F_values", -clutch_size)
# Create scatterplot:
ggplot2::ggplot(mydata, ggplot2::aes(y = clutch_size, x = F_values))+
  ggplot2::geom_point(size = 0.5, alpha = 0.5) +
  ggplot2::geom_smooth(method = "loess") +
  ggplot2::theme_bw() +
  ggplot2::facet_wrap(~F_metric, scales = "free_x")
```

We can see here once again that the *beta0* metrics have more homogeneous distributions than the *beta1* metrics. It is also obvious that direct effects are rather weak (or even null) but an optimistic mind could say that there may be **weak negative relationships** between some of our *F metrics* (e.g. for *d113_beta0* and *beta1*, or for *d531_beta1*, all in log scale).  
...
tpm_xs <- tpm_x[,c("mass", "pmF_d113_beta0", "pmF_d531_beta0", "pmF_d1368_beta0", "pmF_d2848_beta0", "pmF_d113_beta1", "pmF_d531_beta1", "pmF_d1368_beta1", "pmF_d2848_beta1")]
...
mydata <- tpm_xs %>%
  tidyr::gather(key = "F_metric", value = "F_values", -mass)
# Create scatterplot:
ggplot2::ggplot(mydata, ggplot2::aes(y = mass, x = F_values))+
  ggplot2::geom_point(size = 0.5, alpha = 0.5) +
  ggplot2::geom_smooth(method = "loess") +
  ggplot2::theme_bw() +
  ggplot2::facet_wrap(~F_metric, scales = "free_x")
...
Here again, it is clear that the metrics with the two *lowest distances* display smoother relationships with the *outcome* than the other two, regardless of the *beta* parameter. Interestingly, and contrarily to what was visible on the correlation matrix, all *F metrics* seem to have **positive effects** on the nestlings mass, but the relationships seem **straighter** for the *F* computed with *d* = 113 (regardless of its *beta* value) as well as for *F d531_beta0* on the log scale.  
\
\
...
Overall, these observations support our initial preference for the *F metrics* computed with *the two lowest distances* and with *beta* = 0.
\
\


## 3.2. F-metrics for the Blue tits (*Cyanistes caeruleus*)
### 3.2.1. Univariate exploration

Here, you'll notice that contrarily to what was done for *Parus major*, we did not evaluate the *F metrics* computed with *beta* = 2 to gain some time as they would have been discarded anyway.  
...
uni.dotplots(dataset = cc_f[,c(3:10, 2)], COL.PCH = "royalblue3") # To have AREA at the end.
```

It appears all *beta1 F metrics* have extreme values that may be viewed as outliers.
\
...
uni.histograms(dataset = cc_f[,c(3:10, 2)], COL = "aquamarine3") # To have AREA at the end.
```

We can see that, with the exception of the F with ***beta* = 0** and the **largest distance**, most metrics are not close to a normal distribution. This might be problematic.
\
\
...
cc_f %>% dplyr::select(-id_nestbox, -site, -breeding_window, -clutch_size, -brood_size, -fledgling_nb,
                       -mass, -tarsus_length, -wing_length) -> cc_x
...
* First, most predictors are significantly correlated with each others, and most *F metrics* are **positively correlated** with each other, suggesting that they may carry pretty much the same information.
* Second, *woody vegetation cover* and *volume* are **strongly positively correlated** with all *F metrics*, which might be problematic for modelling. A closer look reveals that the correlation is **slightly weaker** with *woodyveg_volume* than with *woody_area*, and that **the lower the distance the higher the correlation**.
* Third, the *F metrics* computed with the **largest distances** (i.e. *d* = 3236) are **extremely positively correlated** with each other BUT LESS SO with the other variables, perhaps because the distance is so high that the metric stops being linked to local environmental conditions. We can also see that they are **negatively correlated** with *longitude* displaying the expected **East-West gradient** of connectivity (a North-South gradient of *F metrics* exists as well).
* Fourth, we could say that, besides the *F metrics* computed with the highest distances, **the highest among F metrics correlations** are between "consecutive" metrics (i.e. metrics with the *same distance* but *differing betas* or with the *same betas* but *differing distances*).
...
```{r ccarea_F ggpairs, fig.align='center', fig.cap="**Figure M6cc.** Bivariate relationship among computed *F-metrics* and some key explanatory variables", fig.width=15, fig.height=12, message=FALSE, comment=NA, warning=FALSE}
cc_x %>% dplyr::select(-strata_div, -age_class, -woody_area, -area, -noise_m, -lsource_vs150_m,
                       -min_t_between) -> cc_fs
GGally::ggpairs(cc_fs)
```

This figure shows patterns relatively similar with those observed for *Parus major*:  

1. For *F metrics* with *beta* = 0, the among metrics correlation is very clear.
2. For *F metrics* with *beta* = 1, patterns are less clear. The correlations are not so straightforward and the 5th and 6th rows show that the most extreme values from *beta1 metrics* **are not the same** than those of the *F metrics* with *beta* = 0, suggesting that they do not entirely share the same information! Interestingly, the among *beta1 metrics* correlations are not so clear.
3. We can confirm that the connectivity increases with decreasing *built* and *open areas*!
4. The last row shows that the sites with **the highest woody volume** are not necessarily those with **the highest *F metrics* **: i.e. although rank-based correlations are extremely high (particularly for metrics with *beta* = 1), the plot shows **clear bivariate outliers** as well as rather **flat relationships** with the *beta0 metrics*, while they display rather **linear relationships** with the *beta1 metrics* yet with enormous gaps in the predictor space.

So, up to this point, we'd be rather inclined into **discarding** *F metrics* based on the *longest distances*, and we have a preference for *F metrics* computed with *beta* = 0.
\
\
...
trainset_cc %>% dplyr::select(id_nestbox, site, breeding_window, clutch_size, brood_size, fledgling_nb,
                       mass, tarsus_length, wing_length, woodyveg_volume,
                       ccF_d92_beta0, ccF_d311_beta0, ccF_d1573_beta0, ccF_d3236_beta0, 
                       ccF_d92_beta1, ccF_d311_beta1, ccF_d1573_beta1, ccF_d3236_beta1) -> trainset_cc
...
1. Unlike for *Parus major*, several *response variables* appear **correlated** to some *F metrics* (according to Spearman's rank correlations).
2. Similarly to PM, correlations seem **negative** for the *reproduction related variables* and **positive** for the *morphometric* ones. 

To further examine these relationships, we plotted the Blue tits couples' *clutch size* and the *mass* of nestlings against all our computed *F metrics*:
...
tcc_xs <- tcc_x[,c("clutch_size", "ccF_d92_beta0", "ccF_d311_beta0", "ccF_d1573_beta0", "ccF_d3236_beta0", 
                   "ccF_d92_beta1", "ccF_d311_beta1", "ccF_d1573_beta1", "ccF_d3236_beta1")]

# Transforming predictors:
tcc_xs <- tcc_xs %>%
  dplyr::mutate("ccF_d92_beta0 (log)" = log(ccF_d92_beta0)) %>%
  dplyr::mutate("ccF_d311_beta0 (log)" = log(ccF_d311_beta0)) %>%
  dplyr::mutate("ccF_d1573_beta0 (log)" = log(ccF_d1573_beta0)) %>%
  dplyr::mutate("ccF_d3236_beta0 (log)" = log(ccF_d3236_beta0)) %>%
  dplyr::mutate("ccF_d92_beta1 (log)" = log(ccF_d92_beta1)) %>%
  dplyr::mutate("ccF_d311_beta1 (log)" = log(ccF_d311_beta1)) %>%
  dplyr::mutate("ccF_d1573_beta1 (log)" = log(ccF_d1573_beta1)) %>%
  dplyr::mutate("ccF_d3236_beta1 (log)" = log(ccF_d3236_beta1)) # These transformations were made to linearise the relationships.
...
We can see here once again that the *beta0* metrics have more homogeneous distributions than the *beta1* metrics. It is also obvious that direct effects are absent!  
To deepen our grasp of the metrics relevance, we also looked at the relationships with the *mass* of nestlings:
...
tcc_xs <- tcc_x[,c("mass", "ccF_d92_beta0", "ccF_d311_beta0", "ccF_d1573_beta0", "ccF_d3236_beta0", 
                   "ccF_d92_beta1", "ccF_d311_beta1", "ccF_d1573_beta1", "ccF_d3236_beta1")]
...
Interestingly, we can see that the *beta0* do not require log-transformations and that **mildly positive** relationship exist with, at least, the *ccF_d311* and *ccF_d1573* metrics, regardless of their *betas*. 
\
\
...
Overall, these observations converges with what we saw for *Parus major* and support our decision to select *beta0 F*s, probably with the second smallest distance value: i.e. the **median gap-crossing distance** given by the respondent to our expert-survey.
\
\
