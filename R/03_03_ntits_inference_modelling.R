...
## Traditional residuals:
par(.pardefault)
plot(ttHSy_glmm3, id = 0.05, idLabels = ~.obs) # Strange pattern.
...
performance::check_outliers(ttHSy_glmm3) # Obs. 58 and 70 as possible outliers (but warnings)!

# To further investigate patterns, I can plot the residuals against some predictors:
resid <- stats::resid(ttHSy_glmm3, type = 'deviance')
...
plot(ttHSy_glmm3, id_nestbox~stats::resid(.)) # Justification for the nestbox random effect (RE).
plot(ttHSy_glmm3, site~stats::resid(.)) # Interestingly, there are not that much among-sites variance.

## Simulation-based scaled residuals computation (DHARMa method):
simu.resid <- DHARMa::simulateResiduals(fittedModel = ttHSy_glmm3, n = 1000, seed = 37)
...
performance::check_autocorrelation(ttHSy_glmm3) # Significant autocorrelation, but the Durbin-Watson test
# is known to be very sensitive.
performance::check_collinearity(ttHSy_glmm3) # Ok-ish, but "urban_intensity" > 4, and 2 others > 3!
stats::vcov(ttHSy_glmm2) # Mostly ok, but there are some high covariances with the intercept.
...
## Assessing over or under-dispersion:
aods3::gof(ttHSy_glmm3) # Note that this test is meant for count data!
...
performance::check_overdispersion(x = ttHSy_glmm3) # Ok-ish.

## Distribution of the predicted probabilities:
probabilities <- stats::predict(object = ttHSy_glmm3, type = "response") # Extract the predicted
# probabilities.
par(mfrow= c(1,2))
hist(probabilities)
...
ttHSy_ziglmm1 <- glmmTMB::glmmTMB(brood_size/clutch_size ~
...
summary(ttHSy_ziglmm1) # Seems to yield better results (e.g. RE variance). AIC = 563.4 (without ZI,
# AIC ~ 713)!
simu.resid_zi <- DHARMa::simulateResiduals(fittedModel = ttHSy_ziglmm1, n = 1000)
...
## Computing a pseudo-R2:
performance::r2_nakagawa(ttHSy_glmm2) # [Additive model]: Marg_R2_glmm = 0.12; Cond_R2_glmm = 0.73.
performance::r2_nakagawa(ttHSy_ziglmm1) # [Additive model]: Marg_R2_glmm = 0.09; Cond_R2_glmm = 0.35 (but
# strange warning, so probably a mistake).

## Likelihood-ration tests (LRT) of GOF:
# Importance of the "id_nestbox" random-effect (RE):
ttHSy_ziglmm0 <- glmmTMB::glmmTMB(brood_size/clutch_size ~
                                    logged_woodyveg + logged_Fmetric +
                                    urban_intensity + manag_low + manag_high + light_pollution + noise_iq +
                                  ...
                                  data = ntits2, family = binomial,
                                  ziformula = ~1)
summary(ttHSy_ziglmm0) # The non-mixed model gives AIC = 589.5 while the mixed-model gave AIC = 563.4, so
# the use of the mixed model seems warranted by the data!

# # Importance of the fixed effects:
# ttHSy_ziglmm0 <- lme4::glmer(brood_size/clutch_size ~ 1 + (1|id_nestbox),
#                            weights = clutch_size, data = ntits2, family = "binomial",
#                            control=lme4::glmerControl(optimizer="bobyqa",
#                                                       optCtrl=list(maxfun=2e5)))
# tictoc::tic("Parametric bootstrap LRT")
# res.LRT_re <- DHARMa::simulateLRT(m0 = ttHSy_ziglmm0, m1 = ttHSy_ziglmm1, n = 500, seed = 85)
