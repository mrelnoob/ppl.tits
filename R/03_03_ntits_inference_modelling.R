...
summary(ttCy_comglm1) # AIC = 1426.5 (vs 1669.5 with the outliers).
summary(ttCy_comglm1b) # AIC = 1427.2 (so it's not exactly the same?!).
summary(ttCy_comglmm1) # AIC = 1408.6 (vs 1626.7 with the outliers).
summary(ttCy_comglmm2) # AIC = 1410.1, so the interaction worsen the fit (vs 1673.5 with the outliers)!
...
# change ntits3 by ntits3 (i.e. the dataset with the deleted outliers).
...
# TEST: variables/proxies, interaction patch_area*urban_intensity OR species??? Or triple patch*F*urban???
# OR replace urban_intensity!!!!

## Replacement of the beta0 "F-metric" (by the beta1 version):
ttCy_comglmm1b <- glmmTMB::glmmTMB(clutch_size ~
                                     scale(log_patch_area, scale = F) * scale(log_F_metric_d2b1, scale = F) *
                                     scale(urban_intensity, scale = F) +
                                     species +
                                     manag_intensity +
                                     light_pollution + noise_m + traffic +
                                     cumdd_30 + year + (1|id_patch),
...
ttCy_comglmm1d <- glmmTMB::glmmTMB(clutch_size ~ logged_woody_area + log_F_metric_d2b1 + species +
                                     urban_intensity + manag_low + manag_high + light_pollution + noise_iq +
                                     cumdd_30 + year + (1|site),
                                   data = ntits3, family = glmmTMB::compois(link = "log"),
                                   dispformula = ~cumdd_30+min_t_before) # Rather long to fit.
...
zzz <- glmmTMB::glmmTMB(clutch_size ~ logged_woody_area + log_F_metric_d2b1 + species +
                                     urban_intensity + manag_low + manag_high + light_pollution + noise_iq +
                                     cumdd_30 + year + (1|site),
                                   data = ntits3, family = glmmTMB::compois(link = "log"),
                                   dispformula = ~cumdd_30+min_t_before) # Rather long to fit.
...
ttCy_comglmm0b <- stats::update(ttCy_comglmm1b, .~. -log_F_metric_d2b1_d2b1)
ttCy_comglmm0c <- stats::update(ttCy_comglmm1c, .~. -log_F_metric_d2b1_d2b1)
ttCy_comglmm0d <- stats::update(ttCy_comglmm1d, .~. -log_F_metric_d2b1_d2b1)
...
# There was no major assumption violations and the use of "id_patch" as RE was warranted by the data, and so
...
ttHSy_ziglmm1 <- glmmTMB::glmmTMB(brood_size/clutch_size ~ log_patch_area + log_F_metric_d2b1 + species +
                                  urban_intensity + manag_intensity + light_pollution + noise_iq +
                                  cumdd_30 + year + (1|id_nestbox),
                                weights = clutch_size, data = ntits2, family = "binomial",
...
## Replacement of the beta0 "F-metric" (by the beta1 version):
ttHSy_glmm1b <- glmmTMB::glmmTMB(brood_size/clutch_size ~ log_patch_area + log_F_metric_d2b1_d2b1 + species +
                                   urban_intensity + manag_intensity + light_pollution + noise_iq +
                                   cumdd_30 + year + (1|id_nestbox),
                                 weights = clutch_size, data = ntits3, family = "binomial")

## Replacement of the woody vegetation volume (by its area) but not the "F-metric':
ttHSy_glmm1c <- glmmTMB::glmmTMB(brood_size/clutch_size ~ logged_woody_area + log_F_metric_d2b1 + species +
                                   urban_intensity + manag_intensity + light_pollution + noise_iq +
                                   cumdd_30 + year + (1|id_nestbox),
                                 weights = clutch_size, data = ntits3, family = "binomial")

## Replacement of both:
ttHSy_glmm1d <- glmmTMB::glmmTMB(brood_size/clutch_size ~ logged_woody_area + log_F_metric_d2b1_d2b1 + species +
                                   urban_intensity + manag_intensity + light_pollution + noise_iq +
                                   cumdd_30 + year + (1|id_nestbox),
                                 weights = clutch_size, data = ntits3, family = "binomial")
## Replacement of "urban_intensity" by "logged_herby_area" but not the "F-metric':
ttHSy_glmm1e <- glmmTMB::glmmTMB(brood_size/clutch_size ~ logged_woody_area + log_F_metric_d2b1 + species +
...
zzz <- glmmTMB::glmmTMB(brood_size/clutch_size ~
                          scale(logged_woody_area, scale = F) * scale(log_F_metric_d2b1_d2b1, scale = F) +
                          species + logged_herby_area + manag_intensity + light_pollution + noise_iq +
                          cumdd_30 + year + (1|id_nestbox),
                        weights = clutch_size, data = ntits3, family = "binomial")
...
ntits3 %>% dplyr::select(log_patch_area, logged_woody_area, woodyveg_sd, logged_woodyvol,
                         log_F_metric_d2b1, log_F_metric_d2b1_d1, log_F_metric_d2b1_d1b1, log_F_metric_d2b1_d2b1,
...
ttHSy_glm0e <- glmmTMB::glmmTMB(brood_size/clutch_size ~ logged_woody_area + log_F_metric_d2b1 + species +
...
## Parametric bootstrap to test the additive effect of the connectivity metric:
ttHSy_glmm0d <- stats::update(ttHSy_glmm1d, .~. -log_F_metric_d2b1_d2b1)
ttHSy_glmm0e <- stats::update(ttHSy_glmm1e, .~. -log_F_metric_d2b1)
...
ttBSy_glm1 <- stats::glm(brood_size ~ logged_woody_area + log_F_metric_d2b1 + species +
                           urban_intensity + manag_low + manag_high + light_pollution + noise_iq +
                           cumdd_30 + year,
                         data = ntits3, family = "poisson")

## Fitting a regular Poisson GLMM:
ttBSy_glmm1 <- glmmTMB::glmmTMB(brood_size ~ logged_woody_area + log_F_metric_d2b1 + species +
                                  urban_intensity + manag_low + manag_high + light_pollution + noise_iq +
                                  cumdd_30 + year + (1|id_nestbox),
                                data = ntits3, family = "poisson")

## Fitting a COM-Poisson GLMM:
ttBSy_comglmm1 <- glmmTMB::glmmTMB(brood_size ~ logged_woody_area + log_F_metric_d2b1 + species +
                                     urban_intensity + manag_low + manag_high + light_pollution + noise_iq +
                                     cumdd_30 + year + (1|id_nestbox),
                                     data = ntits3, family = glmmTMB::compois(link = "log"),
                                     dispformula = ~1) # Rather long to fit.
...
ttBSy_comglmm1b <- glmmTMB::glmmTMB(brood_size ~ logged_woody_area + log_F_metric_d2b1 + species +
                                      logged_herby_area + manag_low + manag_high + light_pollution + noise_iq +
                                      cumdd_30 + year + (1|id_nestbox),
                                    data = ntits3, family = glmmTMB::compois(link = "log"),
                                    dispformula = ~1)
...
ttBSy_comglmm1c <- glmmTMB::glmmTMB(brood_size ~ logged_woody_area + log_F_metric_d2b1 + species +
                                      logged_herby_area + manag_low + manag_high + light_pollution + noise_iq +
                                      cumdd_30 + year + (1|id_nestbox),
                                    data = ntits3, family = glmmTMB::compois(link = "log"),
                                    dispformula = ~log_F_metric_d2b1)

## Trying the beta1 version of the "F-metric":
ttBSy_comglmm1d <- glmmTMB::glmmTMB(brood_size ~ logged_woodyvol + log_F_metric_d2b1_d2b1 + species +
                                      logged_herby_area + manag_low + manag_high + light_pollution + noise_iq +
                                      cumdd_30 + year + (1|id_nestbox),
                                    data = ntits3, family = glmmTMB::compois(link = "log"),
                                    dispformula = ~1)
...
ttBSy_comglmm1e <- glmmTMB::glmmTMB(brood_size ~ logged_woodyvol + log_F_metric_d2b1_d2b1 + species +
                                      logged_herby_area + manag_low + manag_high + light_pollution + noise_iq +
                                      cumdd_30 + year + (1|id_nestbox),
                                    data = ntits3, family = glmmTMB::compois(link = "log"),
                                    dispformula = ~log_F_metric_d2b1)
...
# To further investigate patterns, I can plot the residuals against some predictors:
plot(x = ntits3$log_F_metric_d2b1, y = resid) # There may be signs of heteroscedasticity for the "F-metric".
...
DHARMa::plotResiduals(simu.residc, form = ntits3$log_F_metric_d2b1)
...
ttBSy_comglm1 <- glmmTMB::glmmTMB(brood_size ~ logged_woody_area + log_F_metric_d2b1 + species +
                                     urban_intensity + manag_low + manag_high + light_pollution + noise_iq +
                                     cumdd_30 + year + (1|site),
                                   data = ntits3, family = glmmTMB::compois(link = "log"),
                                   dispformula = ~1) # Rather long to fit.
...
## Parametric bootstrap to test the additive effect of the connectivity metric:
ttBSy_comglmm0 <- stats::update(ttBSy_comglmm1, .~. -log_F_metric_d2b1)
...
ttFSy_ziglmm1a <- glmmTMB::glmmTMB(fledgling_nb/brood_size ~ logged_woody_area + log_F_metric_d2b1_d2b1 +
                                     clutch_size +
                                     urban_intensity + manag_intensity + light_pollution + noise_iq +
                                     cumdd_30 + year + (1|id_obs) + (1|id_nestbox),
                                  weights = brood_size, data = ntits3, family = "binomial",
...
## Using "woodyveg_vw":
ttFSy_ziglmm1b <- glmmTMB::glmmTMB(fledgling_nb/brood_size ~ log_patch_area + log_F_metric_d2b1 +
                                     clutch_size +
                                     urban_intensity + manag_intensity + light_pollution + noise_iq +
                                     cumdd_30 + year + (1|id_obs) + (1|id_nestbox),
                                   weights = brood_size, data = ntits3, family = "binomial",
...
## Using both:
ttFSy_ziglmm1c <- glmmTMB::glmmTMB(fledgling_nb/brood_size ~ log_patch_area + log_F_metric_d2b1_d2b1 +
                                     clutch_size +
                                     urban_intensity + manag_intensity + light_pollution + noise_iq +
                                     cumdd_30 + year + (1|id_obs) + (1|id_nestbox),
                                   weights = brood_size, data = ntits3, family = "binomial",
...
ttFSy_ziglmm1d <- glmmTMB::glmmTMB(fledgling_nb/brood_size ~ logged_woody_area + log_F_metric_d2b1 +
                                    clutch_size +
                                    urban_intensity + manag_intensity + light_pollution + noise_iq +
                                    cumdd_30 + year + (1|id_obs) + (1|id_nestbox),
                                  weights = brood_size, data = ntits3, family = "binomial",
...
ttFSy_ziglmm1e <- glmmTMB::glmmTMB(fledgling_nb/brood_size ~ logged_woody_area + log_F_metric_d2b1_d2b1 +
                                    clutch_size +
                                    urban_intensity + manag_intensity + light_pollution + noise_iq +
                                    cumdd_30 + year + (1|id_obs) + (1|id_nestbox),
                                  weights = brood_size, data = ntits3, family = "binomial",
...
# ntitsMMy_lm1 <- stats::lm(morphometry ~ log_patch_area + log_F_metric_d2b1 +
#                          urban_intensity + cumdd_30 + father_cond + mother_cond + year, data = ntits3)
# # ntitsMMy_lm2 <- stats::lm(morphometry ~ scale(log_patch_area, scale = F) * scale(log_F_metric_d2b1,
# #                                                                                   scale = F) +
# #                            urban_intensity + year, data = ntits3) # Interaction not significant!
#
#
# ## Fitting an additive LMM:
# ntitsMMy_lmm1 <- lme4::lmer(morphometry ~ log_patch_area + log_F_metric_d2b1 + urban_intensity +
...
# ntitsMMy_blmm1 <- blme::blmer(morphometry ~ log_patch_area + log_F_metric_d2b1 + urban_intensity +
#                              cumdd_30 + year + (1|id_nestbox), data = ntits3)
...
# redres::plot_redres(ntitsMMy_blmm3, xvar = "scale(log_F_metric_d2b1, scale = F)") +
#   ggplot2::geom_smooth(method = "loess") +
#   ggplot2::theme_classic() +
...
# # plot(ntits3$log_F_metric_d2b1, stats::residuals(ntitsMMy_blmm3)) # Same plot (I should create a
# # custom function).
...
# ## For the additive effect of the connectivity metric:
# ntitsMMy_blmm1 <- stats::update(ntitsMMy_blmm2, .~. -log_F_metric_d2b1)
#
...
ttTLy_lm1 <- stats::lm(tarsus_length ~ logged_woody_area + log_F_metric_d2b1 +
                         species + clutch_size +
                         urban_intensity + manag_intensity + light_pollution + noise_iq +
                         cumdd_30 + year, data = ntits3)

## Fitting an additive LMM:
ttTLy_lmm1 <- lme4::lmer(tarsus_length ~ logged_woody_area + log_F_metric_d2b1 +
                           species + clutch_size +
                           urban_intensity + manag_intensity + light_pollution + noise_iq +
                           cumdd_30 + year + (1|id_nestbox), data = ntits3,
                         control=lme4::lmerControl(optimizer="bobyqa",
...
redres::plot_redres(ttTLy_lmm1, xvar = "noise_iq") +
  ...
  ggplot2::theme_classic() +
  ggplot2::labs(title = "Residual vs predictor") # Globally ok, but strange pattern for CS?
# plot(ntits3$log_F_metric_d2b1, stats::residuals(ttTLy_lmm1)) # Same plot (I should create a
# custom function).
...
## For the additive effect of the connectivity metric:
ttTLy_lmm0 <- stats::update(ttTLy_lmm1, .~. -log_F_metric_d2b1)
...
ttWLy_lm1 <- stats::lm(wing_length ~ logged_woody_area + log_F_metric_d2b1 +
                            species + clutch_size +
                            urban_intensity + manag_intensity + light_pollution + noise_iq +
                            cumdd_30 + year, data = ntits3)

## Fitting an additive LMM:
ttWLy_lmm1 <- lme4::lmer(wing_length ~ logged_woody_area + log_F_metric_d2b1 +
                           species + clutch_size +
                           urban_intensity + manag_intensity + light_pollution + noise_iq +
                           cumdd_30 + year + (1|id_nestbox), data = ntits3,
                         control=lme4::lmerControl(optimizer="bobyqa",
...
# ttWLy_blmm1 <- blme::blmer(wing_length ~ logged_woody_area + log_F_metric_d2b1 +
#                              species + clutch_size +
#                              urban_intensity + manag_intensity + light_pollution + noise_iq +
#                              cumdd_30 + year + (1|id_nestbox), data = ntits3,
#                            control=lme4::lmerControl(optimizer="bobyqa",
...
## For the additive effect of the connectivity metric:
ttWLy_lmm0 <- stats::update(ttWLy_lmm1, .~. -log_F_metric_d2b1)
