...
#   dplyr::select(repro_success, clutch_size, brood_size, fledgling_nb,
#                 species, coord_x, coord_y, mean_winter_t, sd_winter_t, lsource_vs150_m,
...
#
# ########## FOR PM ###########
# stits %>% dplyr::filter(species == "PM") %>%
#   dplyr::select(-species) -> pm
#
# pm_null.rf <- randomForest::randomForest(repro_success ~ ., data = pm, importance = TRUE,
#                                          nPerm = 2, oob.prox = TRUE, keep.forest = TRUE)
# print(pm_null.rf)
# plot(pm_null.rf)
# randomForest::varImpPlot(pm_null.rf)
#
#
#
# # argument nperm!!!
